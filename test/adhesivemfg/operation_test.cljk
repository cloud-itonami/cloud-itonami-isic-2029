(ns adhesivemfg.operation-test
  "Smoke tests for the compiled AdhesiveOperationActor graph itself
  (build + one happy path per op). The governor's full rule contract
  (HARD holds, escalation, phase gating) is exercised in
  `adhesivemfg.governor-contract-test`; the Store contract in
  `adhesivemfg.store-contract-test`."
...
(deftest test-actor-builds
  (testing "AdhesiveOperationActor can be built with a store"
    ...
    (let [s (-> (store/mem-store) (store/sample-data!))
          ...
          result (exec-op actor "t3"
                          {:op :flag-safety-concern :effect :propose :subject "concern-1"
                           :value {:equipment-id "reactor-001" :severity :moderate :description "溶剤蒸気曝露リスク上昇"}}
                          coordinator)]
      (is (= :interrupted (:status result))))))
