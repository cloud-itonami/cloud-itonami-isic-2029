(ns adhesivemfg.governor-contract-test
  "The governor contract as executable tests -- this vertical's own
  scope boundary ('does NOT control the reactor/mixing or filling/
  packaging line directly... does NOT decide or grant a chemical-
  safety certification') implemented faithfully. The single invariant
  under test:

    AdhesiveAdvisor never schedules maintenance, flags a safety
    concern, or coordinates a shipment the Adhesive Plant Operations
  ...
  (let [[db actor] (fresh)
        res (exec-op actor "t1"
                  {:op :log-production-batch :effect :propose :subject "batch-001"
                   :patch {:product-type :hot-melt-adhesive}} coordinator)]
    (is (= :commit (get-in res [:state :disposition])))
    (is (= :hot-melt-adhesive (:product-type (store/batch db "batch-001"))) "SSoT actually updated")
...
(deftest unknown-op-is-held
  (let [[db actor] (fresh)
        res (exec-op actor "t4" {:op :actuate-reactor :effect :propose :subject "x"} coordinator)]
    (is (= :hold (get-in res [:state :disposition])))
...
(deftest certification-decision-is-held-and-permanently-blocked
  (testing "a proposal that sets :decide-certification? true -> HOLD, PERMANENT, never reaches request-approval -- deciding a chemical-safety certification is exclusively a certification authority's call"
    (let [[db actor] (fresh)
...
(deftest invalid-product-type-is-held
  (let [[db actor] (fresh)
        res (exec-op actor "t10" {:op :log-production-batch :effect :propose :subject "batch-001"
                                  :patch {:product-type :unobtainium-adhesive}} coordinator)]
    (is (= :hold (get-in res [:state :disposition])))
    (is (some #{:invalid-product-type} (-> (store/ledger db) last :basis)))
    (is (not= :unobtainium-adhesive (:product-type (store/batch db "batch-001"))) "fabricated product-type never lands in the SSoT")))
...
(deftest invalid-viscosity-is-held
  (let [[db actor] (fresh)
    ...
    (is (not= -5.0 (:viscosity-cp (store/batch db "batch-001"))) "fabricated viscosity reading never lands in the SSoT")))

(deftest invalid-purity-is-held
  (let [[db actor] (fresh)
        res (exec-op actor "t11b" {:op :log-production-batch :effect :propose :subject "batch-001"
                                   :patch {:purity-pct 150.0}} coordinator)]
    (is (= :hold (get-in res [:state :disposition])))
    (is (some #{:invalid-purity} (-> (store/ledger db) last :basis)))
    (is (not= 150.0 (:purity-pct (store/batch db "batch-001"))) "fabricated purity reading never lands in the SSoT")))

(deftest purity-below-spec-floor-is-held
  (testing "a batch patch whose own declared purity falls below its product type's own regulatory spec floor -> HOLD"
    (let [[db actor] (fresh)
          res (exec-op actor "t11c" {:op :log-production-batch :effect :propose :subject "batch-001"
                                     :patch {:purity-pct 40.0}} coordinator)]
      (is (= :hold (get-in res [:state :disposition])))
      (is (some #{:purity-below-spec-floor} (-> (store/ledger db) last :basis)))
      (is (= 97.0 (:purity-pct (store/batch db "batch-001")))
          "the below-spec-floor purity patch never lands in the SSoT -- batch-001 keeps its seeded purity"))))

(deftest purity-within-spec-floor-auto-commits
  (testing "a batch patch whose own declared purity stays at or above its product type's own regulatory spec floor -> auto-commits cleanly"
    (let [[db actor] (fresh)
          res (exec-op actor "t11d" {:op :log-production-batch :effect :propose :subject "batch-001"
                                     :patch {:purity-pct 96.0}} coordinator)]
      (is (= :commit (get-in res [:state :disposition])))
      (is (= 96.0 (:purity-pct (store/batch db "batch-001")))))))
    ...
    (let [[db actor] (fresh)
          res (exec-op actor "t12" {:op :flag-safety-concern :effect :propose :subject "concern-1"
                                    :value {:equipment-id "reactor-001" :severity :moderate
                                            :description "solvent-vapor exposure risk rising near reaction equipment"}}
                       coordinator)]
      (is (= :interrupted (:status res)))
