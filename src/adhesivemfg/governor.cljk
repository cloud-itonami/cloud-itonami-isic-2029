(ns adhesivemfg.governor
  "Adhesive Plant Operations Governor -- the independent compliance
  layer that earns the AdhesiveAdvisor the right to commit. The
  ...
  reactor/mixing tank or filling/packaging line, whether a proposal
  secretly tries to DECIDE a chemical-safety certification (exclusively
  a certification authority's call), whether a shipment proposal's own
  claimed weight would blow through the batch's own logged production
  weight, or whether a batch's own declared purity actually stays
  above its product type's own regulatory spec floor, so this MUST be
  a separate system able to *reject* a proposal and fall back to HOLD.

  `:itonami.blueprint/governor` is `:adhesive-plant-operations-
  ...
  gate (SOFT -- asks a human to look, and the human may approve):
    ...
    4. Line-actuate blocked        -- for `:schedule-maintenance`, does
                                       ...
                                       reactor/mixing tank or filling/
                                       packaging line is this actor's
    ...
    5. Certification-decision
       blocked                     -- for `:log-production-batch`, does
                                       the proposal's own `:value`
                                       declare
                                       `:decide-certification? true`?
                                       Deciding or granting a
                                       chemical-safety certification
                                       (e.g. GHS/CLP hazard
                                       classification, an SDS
                                       certification, a third-party
                                       product-safety certification
                                       mark) is EXCLUSIVELY a
                                       certification authority's call,
                                       never this actor's -- HARD,
                                       PERMANENT, unconditional, the
                                       domain-specific twin of check 4.
                                       NO phase and NO human approval
                                       can ever override this either.
   ...
   13. Purity below spec floor     -- for `:log-production-batch`,
                                       ...
                                       own declared `:purity-pct`
                                       together fall BELOW that product
                                       type's own closed minimum spec
                                       floor
                                       (`adhesivemfg.registry/purity-
                                       below-spec-floor?`) -- modeled on
                                       representative typical
                                       industrial adhesive product-spec
                                       minimum active-solids-content
                                       ranges, never taken on the
                                       advisor's self-report that the
                                       formulation 'meets spec'.
   14. Confidence floor / high-
       stakes gate                  -- LLM confidence below threshold,
                                       ...
                                       human may approve."
  (:require [adhesivemfg.registry :as registry]
...
(def allowed-proposal-effects
  ...
  mixing-line-equipment-control effect and NEVER a chemical-safety-
...
(defn- line-control-blocked-violations
  ...
  Anything else (direct reactor/mixing-equipment or filling/packaging-
  ...
  (when-not (contains? allowed-proposal-effects (:effect proposal))
    [{:rule :line-control-blocked
      :detail (str "proposal :effect (" (pr-str (:effect proposal))
                   ") は反応槽/混合タンク・充填/包装ラインの直接操作に該当する可能性があり、恒久的に禁止")}]))
...
(defn- line-actuate-blocked-violations
  ...
  directly actuate the reactor/mixing tank or filling/packaging line --
  ...
  (when (and (= op :schedule-maintenance)
    ...
    [{:rule :line-actuate-blocked
      :detail "反応槽/混合タンク・充填/包装ラインの直接操作(actuate)提案は恒久的に禁止 -- 提案(draft)のみ許可"}]))

(defn- certification-decision-blocked-violations
  "HARD, PERMANENT, unconditional: a `:log-production-batch` proposal
  whose own `:value` declares `:decide-certification? true` is
  attempting to have this actor DECIDE or GRANT a chemical-safety
  certification -- that decision is EXCLUSIVELY a certification
  authority's (e.g. a GHS/CLP hazard-classification body, an SDS
  certifier, a third-party product-safety certification mark) call,
  never this actor's. The domain-specific twin of
  ...
  (when (and (= op :log-production-batch)
             (true? (:decide-certification? (:value proposal))))
    [{:rule :certification-decision-blocked
      :detail "化学品安全認証の可否判断・付与は認証機関の専権事項であり、この actor が代行することは恒久的に禁止"}]))
...
(defn- invalid-purity-violations
  "For `:log-production-batch`, if the patch declares a `:purity-pct`
  that is not a physically plausible reading, reject rather than let
  fabricated/sensor-error data through."
  [{:keys [op]} proposal]
  (when (= op :log-production-batch)
    (let [v (:purity-pct (:value proposal))]
      (when (and (some? v) (not (registry/purity-valid? v)))
        [{:rule :invalid-purity
          :detail (str v "% は物理的に妥当な純度の範囲外")}]))))

(defn- purity-below-spec-floor-violations
  ...
  `:purity-pct` falls below that product type's own closed regulatory
  spec floor -- never taken on the advisor's self-report that the
  formulation 'meets spec'. Modeled on representative typical
  industrial adhesive product-spec minimum active-solids-content
  ranges."
  ...
  (when (= op :log-production-batch)
    (let [patch (:value proposal)
          ...
          purity (:purity-pct patch)]
      (when (registry/purity-below-spec-floor? effective-product-type purity)
        [{:rule :purity-below-spec-floor
          :detail (str subject " (product-type=" effective-product-type
                       ") の純度(" purity "%) がスペック下限("
                       (registry/purity-spec-floor-for effective-product-type)
                       "%)を下回る")}]))))

(defn check
  "Censors an AdhesiveAdvisor proposal against the governor rules.
  ...
  (let [hard (into []
                   (concat (no-propose-effect-violations request)
                           ...
                           (certification-decision-blocked-violations request proposal)
                           ...
                           (purity-below-spec-floor-violations request proposal st)))
