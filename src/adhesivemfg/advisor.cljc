(ns adhesivemfg.advisor
  "AdhesiveAdvisor -- the *contained intelligence node* for the ISIC
  2029 (manufacture of other chemical products n.e.c.) plant-
  operations coordination actor, concretely illustrated by
  industrial/consumer adhesives and glues manufacturing.

  It normalizes production-batch patches (product-type/weight/
  viscosity/purity output-quality data), drafts a reactor/mixing-tank
  or filling/packaging-line maintenance scheduling proposal against a
  piece of equipment, drafts a safety-concern flag, and drafts an
  outbound shipment coordination proposal against a production batch.
  CRITICAL: it is a smart-but-untrusted advisor. It returns a
  *proposal* (with a rationale + the fields it cited), never a
  committed record and NEVER a real reactor/mixing-line actuation,
  freight dispatch, or chemical-safety-certification decision. Every
  output is censored downstream by `adhesivemfg.governor` before
  ...
  Proposal shape (all kinds):
    {:summary    str            ; human-facing draft / finding
     :rationale  str            ; why -- informational only, NOT trusted
                                 ...
                                 ; check (see `adhesivemfg.governor`)
  ...
  `adhesivemfg.governor` HARD-holds any request that doesn't, so a
...
(defn- log-production-batch
  ...
  weight, viscosity, purity, or verification status. High confidence,
...
(defn- schedule-maintenance
  "Draft a reactor/mixing-tank or filling/packaging-line maintenance-
  ...
  rationale, but `adhesivemfg.governor` NEVER trusts this report -- it
...
(defn- flag-safety-concern
  "Draft a chemical-handling/reaction-hazard (toxicity/exposure-risk/
  flammability/reactive-hazard) or equipment-safety concern. ALWAYS
  ...
  `adhesivemfg.phase`: no phase ever adds this op to a phase's `:auto`
  set; `adhesivemfg.governor` also always escalates on
...
(defn- coordinate-shipment
  "Draft an outbound adhesive/glue-product shipment coordination
  proposal against a production batch. The advisor passes through the
  caller's own claimed weight -- it does NOT invent one, and
  `adhesivemfg.governor` NEVER trusts it: it independently recomputes
...
(def ^:private system-prompt
  (str "あなたはその他の化学製品(接着剤・粘着剤)製造工場プラント運用"
       "コーディネーターの助言者です。与えられた事実のみに基づき、"
       ...
       "いけません。反応槽・混合タンク・充填/包装ライン等の設備の直接"
       "操作(actuate)を絶対に提案してはいけません(この actor は提案の"
       "みを行い、実行は一切行いません)。出荷量を偽って報告しては"
       "いけません。化学品安全認証の可否を判断・付与してはいけません"
       "(この actor は認証機関ではありません)。純度がスペック下限を"
       "下回るバッチを適合と偽って報告してはいけません。"))
...
(defn- parse-proposal
  ...
  yields a safe low-confidence noop so `adhesivemfg.governor`
...
(defn trace
  ...
  [request proposal]
  {:t          :adhesivemfg-advisor-proposal
   :op         (:op request)
