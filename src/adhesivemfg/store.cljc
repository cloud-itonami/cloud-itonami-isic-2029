(ns adhesivemfg.store
  "SSoT for the ISIC 2029 (manufacture of other chemical products
  n.e.c.) plant-operations coordination actor, concretely illustrated
  by industrial/consumer adhesives and glues manufacturing, behind a
  ...
  (`cloud-itonami-isic-2021`'s own `pesticidemfg.store`), this build
  ...
  self-contained (no external adhesive/glue-manufacturing capability
  ...
  Four kinds of entity live here:
    - `batches`          -- the central entity. A production batch's
                             product-type/weight/viscosity/purity
    ...
    - `equipment`         -- a reactor or mixing-tank unit's own
                             ...
                             `batches`.
    - `maintenance`       -- a scheduled reactor/mixing/filling/
                             packaging-line maintenance-window DRAFT
                             against a piece of equipment
                             (`adhesivemfg.registry`'s
    ...
    - `shipments`         -- a proposed outbound adhesive/glue-product
                             shipment DRAFT (`adhesivemfg.registry`'s
                             `register-shipment`).
...
(defprotocol Store
  ...
  (ledger [s])
  (maintenance-history [s] "the append-only maintenance-schedule history (adhesivemfg.registry drafts)")
  (shipment-history [s] "the append-only shipment-coordination history (adhesivemfg.registry drafts)")
...
(defn- sample-batches []
  {"batch-001" {:id "batch-001" :product-type :hot-melt-adhesive :output-form :solid-pellet
                :material "EVA Hot-Melt Adhesive"
                :weight-kg 5000.0 :viscosity-cp 45000.0 :purity-pct 97.0
                :verified? true :registered? true
                :shipped-weight-kg 1000.0
                :last-assessed "2026-06-01"}
   "batch-002" {:id "batch-002" :product-type :water-based-adhesive :output-form :liquid
                :material "PVA Water-Based Wood Glue"
                :weight-kg 8000.0 :viscosity-cp 3000.0 :purity-pct 45.0
                :verified? true :registered? true
                :shipped-weight-kg 7500.0
                :last-assessed "2026-06-01"}
   "batch-003" {:id "batch-003" :product-type :epoxy-adhesive :output-form :two-part-resin
                :material "Structural Epoxy Adhesive"
                :weight-kg 2000.0 :viscosity-cp 80000.0 :purity-pct 96.0
                :verified? false :registered? false
                :shipped-weight-kg 0.0
                :last-assessed "2026-05-15"}})

(defn- sample-equipment []
  {"reactor-001" {:id "reactor-001" :kind :reactor
                  :verified? true :registered? true
                  :last-maintenance-date "2026-05-01"}
   "mixer-002" {:id "mixer-002" :kind :mixing-tank
                :verified? false :registered? false
...
(defn- schedule-maintenance!
  ...
  maintenance-schedule record via `adhesivemfg.registry` and returns
...
(defn- propose-shipment!
  ...
  shipment-coordination record via `adhesivemfg.registry` and returns
  {:result .. :patch ..} for the caller to persist."
  [s shipment-id]
...
(defn sample-data!
  ...
  set -- two verified+registered batches within their own purity spec
  floor (one with shipping headroom, one nearly fully shipped so a
  small new shipment blows through its own logged weight -- HARD
  hold), one UNVERIFIED/unregistered batch (blocks any shipment
  coordinated against it); one verified+registered reactor unit
  (schedulable for maintenance), one UNVERIFIED/unregistered
  mixing-tank unit (blocks any maintenance scheduling against it) --
