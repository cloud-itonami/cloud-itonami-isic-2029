(ns adhesivemfg.operation
  "AdhesiveOperationActor -- one plant-operations coordination request
  = one supervised actor run, expressed as a langgraph-clj StateGraph.
  The advisor (AdhesiveAdvisor) is sealed into a single node
  (:advise); its proposal is ALWAYS routed through the Adhesive Plant
  ...
  (:require [langgraph.graph :as g]
            ...
            [adhesivemfg.advisor :as advisor]
...
(defn build
  "Compiles an AdhesiveOperationActor graph bound to `store`.
  opts:
    :advisor      -- an `adhesivemfg.advisor/Advisor` (default: mock-advisor)
  ...
  (-> (g/state-graph
      ...
      ;; AdhesiveAdvisor inference (the contained intelligence node) -- proposal only.
      (g/add-node :advise
        (fn [{:keys [request]}]
          (let [p (advisor/-advise advisor store request)]
            {:proposal p :audit [(advisor/trace request p)]})))

      ;; Adhesive Plant Operations Governor -- independent censor.
      (g/add-node :govern
