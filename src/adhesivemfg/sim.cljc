(ns adhesivemfg.sim
  ...
  production weight, a proposal that tries to ACTUATE the reactor/
  mixing or filling/packaging line directly (permanently blocked, no
  override), a proposal that tries to DECIDE a chemical-safety
  certification directly (permanently blocked, no override), a
  ...
  an implausible viscosity reading, a production-batch patch with an
  implausible purity reading, and a production-batch patch whose
  declared purity falls below its product type's own regulatory spec
  floor.
...
(defn -main [& _args]
  (let [db (-> (store/mem-store) (store/sample-data!))
    ...
    (println "== schedule-maintenance mnt-1 on reactor-001 (verified, registered, reactor -- escalates, approve) ==")
    ...
    (println "== schedule-maintenance mnt-2 on mixer-002 (UNVERIFIED/unregistered mixing tank -> HARD hold) ==")
    ...
    (println "== schedule-maintenance mnt-3 on reactor-001 with :actuate-line? true -> HARD hold, PERMANENT, never reaches a human ==")
    ...
    (println "== log-production-batch batch-001 with an implausible purity reading -> HARD hold ==")
    (println (exec-op actor "t14"
                       {:op :log-production-batch :effect :propose :subject "batch-001"
                        :patch {:purity-pct 150.0}}
                       coordinator))

    (println "== log-production-batch batch-001 with purity falling below the hot-melt-adhesive spec floor -> HARD hold ==")
